...
class Calculator {
    ...
    func add(lhs: (Int, Int), rhs: (Int, Int)) -> (Int, Int) {
        let lResult = lhs.0 + rhs.0
        let rResult = lhs.1 + rhs.1
    ...
    func mathOp(args: [Int], beg: Int, op : (Int, Int) -> Int) -> Int {
        var currentVal = op(beg, args[0])
        for i in 1...args.count - 1{
            currentVal = op(currentVal, args[i])
        }
        print(currentVal)
        return currentVal
    }
...
// I wrote separate test cases for anonymous function calls, as the right side does not show the boolean value but rather the number of times the anonymous function is called. This is to show that the mathOp outputs are correct.

var res = calc.mathOp(lhs: 5, rhs: 5, op: { (lhs: Int, rhs: Int) -> Int in (lhs + rhs) + (lhs * rhs) }) == 35
res
// This style is one way of writing an anonymous function
res = calc.mathOp(lhs: 10, rhs: -5, op: { ($0 + $1) + ($0 - $1) }) == 20
    // This is the second, more terse, style; either works
res
res = calc.mathOp(args: [1, 2, 3], beg: 0, op: { $0 + $1 }) == 6
res
res = calc.mathOp(args: [1, 2, 3, 4, 5], beg: 0, op: { $0 + $1 }) == 15
res
    // this is (((((0 op 1) op 2) op 3) op 4) op 5)
res = calc.mathOp(args: [1, 1, 1, 1, 1], beg: 1, op: { $0 * $1 }) == 1
res
